...
static u8 CreateNPCTrainerParty(struct Pokemon *party, u16 trainerNum, bool8 firstTrainer)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            case F_TRAINER_PARTY_STATUS:
            {
                const struct TrainerMonItemStatusCustomMoves *partyData = gTrainers[trainerNum].party.ItemStatusMoves;
				...
				SetMonData(&party[i], MON_DATA_ABILITY_NUM, &partyData[i].abilityNums);
				SetMonData(&party[i], MON_DATA_STATUS, &partyData[i].status);
				SetMonData(&party[i], MON_DATA_HP, &partyData[i].hp);
                break;
            }
            }
        }
